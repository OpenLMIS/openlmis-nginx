...
export NGINX_LOG="${NGINX_LOG:-/var/log/nginx}"
export CONSUL_TEMPLATE_LOG="${CONSUL_TEMPLATE_LOG:-/var/log/consul-template}"
...
consul-template \
  -consul "$CONSUL_PATH" \
  -template "$INPUT_FILE:$OUTPUT_FILE:$CALLBACK" >> "${CONSUL_TEMPLATE_LOG}/ctmpl.log" 2>&1 &
